...
uint8_t Gpu2C02::cpuBusRead(const uint16_t addr, const bool bReadOnly){
	uint8_t data = 0x00;
	if (bReadOnly) {	//debugging mode for (viewing unmodified reg contents)
		switch (addr)
		{
		case 0x0000:	//Control
			data = control.reg;
	...
	else {				//though CPU reads from bus some data/reg content is updated
		switch (addr)
		{
		...
		case 0x0002:	//Status
			...
			address_latch = 0;
			break;
		case 0x0003:	//OAM addr (mapper) - not readable
			break;
		case 0x0004:	//OAM data (mapper)
			data = pOAM[oam_addr];
			break;
		case 0x0005:	//Scroll - not readable
			break;
		case 0x0006:	//Gpu Addr - not readable
			break;
		case 0x0007:	//Gpu Data 
...
uint8_t Gpu2C02::gpuBusRead(uint16_t addr, const bool bReadOnly){
	uint8_t data = 0x00;
	addr &= 0x3FFF;
	if (cartridge->gpuBusRead(addr, data)) {}		//if for cartridge, then skip all next
	...
	else throw new std::exception;	//should never occur
	return data;
}

void Gpu2C02::gpuBusWrite(uint16_t addr, const uint8_t data) {
	addr &= 0x3FFF;									
	if (cartridge->gpuBusWrite(addr, data)) {}		//if for cartridge, then skip all others 
	else if (addr >= 0x0000 && addr <= 0x1FFF) {	//pattern Ram
...
void Gpu2C02::connectCartridge(const std::shared_ptr<Cartridge>& cartridge){ this->cartridge = cartridge; }
...
void Gpu2C02::clock() {
	...
	if (scanline >= -1 && scanline < 240) {
		...
		if (scanline == -1 && cycle == 1) {	//new Frame Draw
			status.vertical_blank = 0;		//reset
			status.sprite_zero_hit = 0;		//reset
			status.sprite_overflow = 0;		//reset

			for (int i = 0; i < 8; i++) {	//reset
				//clear out shifters (pixel is transparent(Nulls/0x00) and no rendering is reqd)
		...
		if ((cycle >= 2 && cycle < 258) || (cycle >= 321 && cycle < 338)) {
			...
			*/
			...
			case 0:
				...
				break;
			case 2:
				bg_next_tile_attrib = gpuBusRead((0x2000 + 0x03C0)	//nametable + attribs offset
		...
		if (cycle == 256) IncrementScrollY(); //set next Scanline
		if (cycle == 257) {					  //reset for next scanline cycles
			LoadBackgroundShifters();
			TransferAddressX();
		}
		...
		if (scanline == -1 && cycle >= 280 && cycle < 305) //End of v_blank period(and frame)
			TransferAddressY();
	...
	if (scanline >= 241 && scanline < 261) {
		if (scanline == 241 && cycle == 1) {
			...
			if (control.enable_nmi)	this->nmi = true;	//set for updating cpu 
		}
	}
	...
	//check TV scanning for more details

	//GPU renderer internal clock (never stops painting screen)		
	cycle++;
	...
	if (cycle >= 341) {					//cols done?
		...
		if (scanline >= 261) {			//rows done?
			...
			frame_complete = true;		//mark current frame, Complete!
			odd_frame = !odd_frame;		//toggle odd/even frame
		}
	}
}
	...
	for (uint16_t nTileY = 0; nTileY < 16; nTileY++) {
		for (uint16_t nTileX = 0; nTileX < 16; nTileX++) {
			...
			for (uint16_t row = 0; row < 8; row++) {
				uint8_t tile_lsb = gpuBusRead(i * 0x1000 + nOffset + row + uint16_t(0));
				uint8_t tile_msb = gpuBusRead(i * 0x1000 + nOffset + row + uint16_t(8));
