...
uint8_t Cpu6502::GetFlag(CPUFlags f)
{
	return ((status & f) > 0) ? 1 : 0;
}

// Sets or clears a specific bit of the status register
void Cpu6502::SetFlag(CPUFlags f, bool v)
{
	if (v)
		status |= f;
	else
...
std::map<uint16_t, std::string> Cpu6502::disassemble(uint16_t nStart, uint16_t nStop)
{
	...
	while (addr <= (uint32_t)nStop)
	{
		...
		uint8_t opcode = bus->cpuBusRead(addr, true); addr++;
		...
		{
			value = bus->cpuBusRead(addr, true); addr++;
			sInst += "#$" + hex(value, 2) + " {IMM}";
		}
		else if (lookup[opcode].addrmode == &Cpu6502::ZP0)
		{
			lo = bus->cpuBusRead(addr, true); addr++;
			hi = 0x00;												
			sInst += "$" + hex(lo, 2) + " {ZP0}";
		}
		...
		{
			lo = bus->cpuBusRead(addr, true); addr++;
			hi = bus->cpuBusRead(addr, true); addr++;
