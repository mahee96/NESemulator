...
bool Mapper_004::cpuMapRead(uint16_t addr, uint32_t &mapped_addr, uint8_t &data) {
	if (addr >= 0x6000 && addr <= 0x7FFF) {
		...
		data = vRAMStatic[addr & 0x1FFF];	// Write data to RAM
...
bool Mapper_004::gpuMapRead(uint16_t addr, uint32_t &mapped_addr){
...
bool Mapper_004::gpuMapWrite(uint16_t addr, uint32_t &mapped_addr){	return false; }
